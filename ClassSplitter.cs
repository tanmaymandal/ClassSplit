...
public class ClassSplitter
{
    ...
    public void SplitSingleClass(ClassInfo classInfo, SplitOptions options, List<string> globalUsingStatements)
    {
        ...
        if (_config.AppSettings.BackupOriginalFile)
        {
            CreateBackup(options.InputFilePath);
        }
        ...
        {
            var outputFileName = GenerateFileName(classInfo.ClassName, i + 1);
    ...
    private void GeneratePartialClassFile(ClassInfo classInfo, List<MethodInfo> methods, string outputPath, List<string> globalUsingStatements)
    {
        ...
        // Add generated comment if configured
        if (_config.OutputSettings.AddGeneratedComment)
        {
            content.Add("// <auto-generated>");
            content.Add($"// This file was generated by SplitCS on {DateTime.Now:yyyy-MM-dd HH:mm:ss}");
            content.Add("// Changes to this file may be lost if the code is regenerated.");
            content.Add("// </auto-generated>");
            content.Add("");
        }
        ...
        for (int i = 0; i < methods.Count; i++)
        {
            ...
            foreach (var line in methodLines)
            {
                ...
                {
                    // Ensure proper indentation based on configuration
                    var trimmedLine = line.TrimStart();
                    if (trimmedLine.Length > 0)
                    {
                        var indentChar = _config.OutputSettings.UseSpaces ? ' ' : '\t';
                        var indentSize = _config.OutputSettings.UseSpaces ? _config.OutputSettings.IndentSize : 1;
                        var indent = new string(indentChar, indentSize);
                        content.Add(indent + trimmedLine);
                    }
                    else
                    {
                        content.Add("");
    ...
    private void CreateBackup(string originalFilePath)
    {
        ...
        var backupPath = $"{originalFilePath}.backup_{DateTime.Now:yyyyMMdd_HHmmss}";
        Console.WriteLine($"    → Creating backup: {Path.GetFileName(backupPath)}");
        File.Copy(originalFilePath, backupPath);
        Console.WriteLine($"    ✓ Backup created");
    }

    private string GenerateFileName(string className, int partNumber)
    {
        var pattern = _config.OutputSettings.FileNamingPattern;
        return pattern
            .Replace("{ClassName}", className)
            .Replace("{PartNumber}", partNumber.ToString());
    }
}